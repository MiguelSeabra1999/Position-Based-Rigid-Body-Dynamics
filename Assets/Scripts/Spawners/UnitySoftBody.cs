...
public class UnitySoftBody : MonoBehaviour
{
    ...
    public Rigidbody[,,] particles = new Rigidbody[2, 2, 2];
    ...
    private void Init()
    {
        ...
        particles = new Rigidbody[dims.x + 1, dims.y + 1, dims.z + 1];
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.P))
            foreach (Rigidbody p in particles)
            {
                p.position = new Vector3(p.position.x, 0.5f, p.position.z);
            }
    }
...
*/
    ...
    private Rigidbody SpawnParticle(Vector3 pos)
    {
        GameObject obj = Instantiate(prefabSO.unitySphere, pos , Quaternion.identity);
        obj.transform.SetParent(transform.parent);
        obj.transform.localScale = new Vector3(0.25f, 0.25f, 0.25f);
        obj.GetComponent<SphereCollider>().radius = 0.125f;
    ...
    private void AddDistanceConstraint(Vector3Int a, Vector3Int b)
    {
        ...
        SpringJoint c = particles[a.x, a.y, a.z].gameObject.AddComponent<SpringJoint>();

        c.connectedBody = particles[b.x, b.y, b.z];
        c.autoConfigureConnectedAnchor = false;
        c.anchor = new Vector3(0, 0, 0);
        c.connectedAnchor = new Vector3(0, 0, 0);
        if (compliance == 0)
            c.spring = 3.402823e+38f;
        else
            c.spring = 1 / compliance;
        c.tolerance = 0;
        c.damper = 0;
